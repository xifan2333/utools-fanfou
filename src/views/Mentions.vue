	...
	<div class="mentions">
		...
		<div class="mentions" v-if="mentions">
			<v-card
				...
				v-for="(m, index) in mentions"
				:index="`menu-item-${index}`"
				:key="`menu-item-${m.id}}`"
			>
				<v-list-item>
					<v-list-item-avatar>
						<v-img :src="m.user.profile_image_url"></v-img>
					</v-list-item-avatar>
					<v-list-item-content>
				...
				<div class="d-flex flex-no-wrap justify-space-between">
					<div>
						<v-card-subtitle v-html="m.text"></v-card-subtitle>
					</div>
					<v-avatar
						v-if="m.photo"
						class="mr-4"
						size="125"
						tile
						@click.stop="showImage(m.photo.originurl)"
					>
						<v-img :src="m.photo.originurl"></v-img>
					</v-avatar>
				</div>
				<v-card-actions>
					<span class="fromNow">{{ fromNow(m.created_at) }}</span>
					<v-spacer></v-spacer>
					<v-btn icon @click="repost(m)">
						<v-icon>mdi-comment-arrow-left</v-icon>
					</v-btn>
					<v-btn icon @click="reply(m)">
